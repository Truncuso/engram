...
| 14. SC-11 (OQ-08): start a dream job; `SIGKILL` the worker mid-run; assert the job reaches a **terminal recoverable state** — `FAILED` via the dead-PID crash-recovery scan (kill -9 ⇒ dead PID), *or* `TIMED_OUT` if detected by the stale-heartbeat watchdog first; call `dream.trigger({name, resumeJobId})`; assert new job reaches MERGED; assert staging watermark unchanged (no observation loss). Add a companion case: freeze the worker (SIGSTOP, live-but-hung) ⇒ `TIMED_OUT` | `tests/e2e/sc11-worker-crash-recovery.test.ts` | TODO |
...
| SC-11 | Worker crash -> terminal recoverable -> idempotent resume | Killed worker → FAILED (dead-PID) or TIMED_OUT (stale heartbeat); hung worker → TIMED_OUT; resume succeeds; staging watermark unchanged (OQ-08) | `sc11-worker-crash-recovery.test.ts` |
