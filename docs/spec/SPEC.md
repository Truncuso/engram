---
title: "engram — Agentic Memory System — SPEC v2"
project: engram
repo: github.com/Truncuso/engram
version: v2 (2026-05-22)
status: APPROVED — implementable; folds Round-1 review + Round-2 research findings
supersedes: SPEC v1 (2026-05-22 draft) — see git history
inputs:
...
This is **SPEC v2**: a rewrite of v1 folding in two parallel review rounds
(security, architecture, failure-safety, observability) and four research
reports (QMD/graphify, agentmemory, MCP/Agent-SDK, memory reliability). It is
intended to be implementable end-to-end.
...
interface GraphPlugin extends PluginLifecycle {
  ingestEdges(refs: MemoryRef[]): Promise<void>;
...
- `m_v(origin, confidence)` table (R-1):

  | origin | default `confidence` | default `m_v` |
  |--------|----------------------|---------------|
  | human | 0.90 | **1.0** |
  | ingested | 0.80 | **0.9** |
...
- `verification_state: human-verified` overrides `m_v` to **0.95** (via
  `engram memory confirm <id>`).
...
trigger:
  cumulative_importance: 50     # R-2 — also fire when staging accumulates this much
```
...
2. **Connect** — discover latent relations, insert `[[links]]`. Detect
   emergent entities (≥3 mentions, no dedicated memory).
   **Two-layer contradiction detection** (R-4): semantic
   (`sim>0.85 ∧ |Δimportance|>0.3`) + graph (`contradicts` edges). On hit:
   manifest entry `action: queue_review`, never auto-resolve.
...
```json
{
  ...
  "changes": [
    ...
    { "kind": "lifecycle", "id": "mem_01HY...",
      ...
      "score_breakdown": {"importance":0.05,"recency":0.02,"access":1} },
...
**Rules:**
1. **Atomic claim** — `queued→running` + `pid` write in one SQLite
   transaction. No double-spawn.
...
5. **Merge OCC** (F-5.3) — orchestrator re-validates each hunk against base
   versions in the manifest. Field-disjoint changes get **three-way merge**;
   field-overlap → review queue.
...
| Change kind | Default |
|-------------|---------|
| new file (validates against schema) | **auto-safe** |
| new link / `relations` addition | **auto-safe** |
...
Claude Code first. Hooks `PostToolUse`, `PostToolUseFailure`, `Stop`,
`SessionEnd`, `UserPromptSubmit`, `PreCompact` (re-inject memory before
compaction), `SubagentStart/Stop`. Each hook is fire-and-forget:
...
to the LLM. Mitigation: configure a local Ollama model as graphify's backend
to keep cost local and remove network dependency.
...
- **AppLog** (SQLite, always-on) — `.engram/app-log.db`. Per-field provenance.
  ...
  `mcp_call`, `capture`, `dream_run`, `ingest_run`, `occ_conflict`, `migration`.
...
4. Plugin init order: LLM → Retrieval → Graph. **LLM init failure = FATAL.**
   Retrieval failure = degraded mode (grep fallback). Graph failure = WARN,
   graph-dependent features disabled.
...
engram log --denied                  # denied MCP calls (audit)
...
| Retrieval plugin | **QMD library (`@tobilu/qmd` ≥2.5.1)** | research §1; in-process if Node-22 library mode confirmed, else MCP subprocess |
| Graph plugin | **graphify (subprocess + JSON-RPC stdio)** | research §2; with local Ollama backend for Markdown extraction |
| LLM substrate | **Vercel AI SDK (`ai` package)** behind `LlmPlugin` | research §6 |
...
| OQ-D (store paths) | OPEN | `~/.engram/` global, `<repo>/.engram/` project — confirm in plan |
...
| OQ-G (raw/ git-LFS vs ignore) | OPEN | ignore default, LFS opt-in |
...
| OQ-L (AppLog git-tracked) | OPEN | yes default |
